...
In the `lambdas/` directory, there are three AWS Lambda Functions. A forth is defined in [`/terraform/lambda/`](./terraform/lambda/).
...
> [!NOTE]
> This Lambda Function is not currently deployed.
...
The Lambda Function creates a pull request from a fork owned by the tna-pronom GitHub user with the relevant changes.
