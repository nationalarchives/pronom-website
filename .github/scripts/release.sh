...
S3_URL="$ENVIRONMENT-pronom-website"

docker compose up -d --build
docker compose cp nginx:/usr/share/nginx/html/ .
docker compose exec pronom poetry run python .github/scripts/generate_index_file.py /home/app/pronom-signatures
...
docker compose exec pronom poetry run python .github/scripts/generate_version_file.py "$LATEST_SIGNATURE_FILE"
