...
def get_json_files(token, branch_name):
    ...
    with zipfile.ZipFile(zip_stream) as zip_file:
        ...
        actor_files = [name for name in file_names if '/actors/' in name and name.endswith('.json')]
        signature_json = {'/'.join(name.split('/')[2:]): json.loads(zip_file.read(name)) for name in signature_files}
        actor_json = {'/'.join(name.split('/')[2:]): json.loads(zip_file.read(name)) for name in actor_files}
...
def create_pull_request(token, message, path, content_bytes, branch_name, author_name, body_text):
    api = GhApi(token=token)
