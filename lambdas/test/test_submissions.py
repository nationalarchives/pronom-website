...
class SubmissionsTest(unittest.TestCase):
    ...
    def assert_pr_not_created(self, mock_gh_api, form_body, signature, actor):
        ...
        mock_instance.repos.download_zipball_archive.return_value = (
            self.create_zip_file(signature, actor)
        )
        submissions.lambda_handler({"body": form_body}, None)
        self.assertEqual(
            mock_instance.repos.create_or_update_file_contents.call_count, 0
        )
    ...
    def get_pr_file_contents(self, mock_gh_api, form_body, signature, actor):
        ...
        create_pr_arguments = (
            mock_instance.repos.create_or_update_file_contents.mock_calls[0].kwargs
        )
    ...
    def test_edit_format_existing_field(self, _, mock_gh_api):
        ...
        pr_file_content = self.get_pr_file_contents(
            mock_gh_api, form_body, signature, {}
        )
        self.assertEqual(pr_file_content["formatName"], "testName")
    ...
    def test_edit_add_to_existing_identifiers(self, _, mock_gh_api):
        ...
        signature = {
            "identifiers": [
                {"identifierText": "existingText", "identifierType": "existingType"}
            ]
        }
        pr_file_content = self.get_pr_file_contents(
            mock_gh_api, form_body, signature, {}
    ...
    def test_edit_add_to_missing_relationships(self, _, mock_gh_api):
        ...
        pr_file_content = self.get_pr_file_contents(
            mock_gh_api, form_body, {"fileFormatID": 1}, {}
        )
    ...
    def test_edit_add_to_existing_relationships(self, _, mock_gh_api):
        ...
        signature = {
            "fileFormatID": 1,
            "relationships": [
                {"relatedFormatID": 2, "relationshipType": "existingType"}
            ],
        }
        pr_file_content = self.get_pr_file_contents(
            mock_gh_api, form_body, signature, {}
        ...
        for field_value in ["0", "2"]:
            for actor_field in ["developedBy", "supportedBy"]:
                with self.subTest(actor_field):
                    ...
                    signature = {}
                    if field_value == "0":
                        self.assert_pr_not_created(
                            mock_gh_api, form_body, signature, {}
                        )
                    else:
                        pr_file_contents = self.get_pr_file_contents(
                            mock_gh_api, form_body, signature, {}
                        )
                        self.assertEqual(
                            pr_file_contents[actor_field], int(field_value)
                        )
    ...
    def test_add_actor(self, _, mock_gh_api):
        form_body = (
            "submissionType=add-actor&name=testName&address=testAddress&addressCountry=GBR"
    ...
    def test_edit_actor_change_existing_property(self, _, mock_gh_api):
        ...
        pr_file_contents = self.get_pr_file_contents(
            mock_gh_api, form_body, {}, {"name": "originalName"}
        )
        self.assertEqual(pr_file_contents["name"], "testName")
    ...
    def test_edit_actor_add_new_property(self, _, mock_gh_api):
        ...
        pr_file_contents = self.get_pr_file_contents(
            mock_gh_api, form_body, {}, {"name": "testName"}
        )
        self.assertEqual(pr_file_contents["name"], "testName")
        self.assertEqual(pr_file_contents["address"], "testAddress")
    ...
    def test_add_format_adds_a_new_format_with_signature(self, _, mock_gh_api):
        form_body = (
            "submissionType=add-format&signature-byteSequence=testSequence&signature-maxOffset=0&signature"
    ...
    def test_correct_change_description_is_used_to_create_pr(self, _, mock_gh_api):
        for change_description in ["A change", None]:
            with self.subTest(change_description):
                mock_gh_api.reset_mock()
                description_request = (
                    f"&changeDescription={change_description}"
                ...
                self.assertEqual(
                    pr_args["body"],
                    "New submission" if not change_description else change_description,
                )
    ...
    def test_add_sha_if_file_exists(self, _, mock_gh_api):
        ...
        submissions.lambda_handler({"body": form_body}, None)
        pr_args = mock_instance.repos.create_or_update_file_contents.mock_calls[
            0
        ].kwargs
    ...
    def test_error_with_invalid_submission_type(self, _, mock_gh_api):
        ...
        self.assertEqual(
            response["body"], "An error occurred: submissionType invalid not found"
        )
