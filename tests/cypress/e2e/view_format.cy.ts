describe("PRONOM View Format Spec", () => {
  ...
  it("renders the summary section", () => {
    cy.visit("fmt/1509");
    cy.get("h1.tna-hgroup__title").should(
      "have.text",
    ...
    cy.get(".tna-dl--lined > dt")
      ...
      .then((_) => {
        ...
        expect(summaryFields["Source"]).to.equal(
  ...
  it("renders the containers section", () => {
    ...
    cy.get(".tna-dl")
      ...
      .then((_) => {
        ...
        expect(containerFields["Sequence"]).to.equal(
          "'Visio (TM) Drawing'0D0A",
        );
        expect(containerFields["Right fragment"]).to.equal(
          "Max offset 6 Min offset 6 Position 1",
        );
      });
  });
  ...
  it("renders the signature section", () => {
    ...
    cy.get("#internal-signatures-1559 > h3").should(
      "have.text",
    ...
    cy.get(".tna-dl--lined > dt")
      ...
      .then((_) => {
        expect(signatureFields["Min Frag Length"]).to.equal(
          "Absolute from BOF",
        );
  ...
  it("renders the sidebar", () => {
    ...
    cy.get('[href="#container-signatures-1030"]').should(
      "have.text",
      "Microsoft Word OOXML",
    );
    cy.get('[href="#container-signatures-1040"]').should(
      "have.text",
      "Microsoft Word OOXML (little endian unicode)",
    );
    cy.get('[href="#container-signatures-1050"]').should(
      "have.text",
      "Microsoft Word OOXML (big endian unicode)",
    );
  });
  ...
  it("renders the relationships", () => {
    ...
    cy.get("#relationships + dl > dd a")
      .each((a) => {
        relationships[a.attr("href") as string] = a.text();
      })
      .then((_) => {
        expect(relationships["/fmt/42"]).to.equal(
          "JPEG File Interchange Format 1.00",
        );
        expect(relationships["/fmt/43"]).to.equal(
          "JPEG File Interchange Format 1.01",
        );
        expect(relationships["/fmt/44"]).to.equal(
          "JPEG File Interchange Format 1.02",
        );
        expect(relationships["/fmt/112"]).to.equal(
          "Still Picture Interchange File Format 1.0",
        );
        expect(relationships["/x-fmt/390"]).to.equal(
          "Exchangeable Image File Format (Compressed) 2.1",
        );
        expect(relationships["/x-fmt/391"]).to.equal(
          "Exchangeable Image File Format (Compressed) 2.2",
        );
        expect(relationships["/x-fmt/398"]).to.equal(
          "Exchangeable Image File Format (Compressed) 2.0",
        );
        expect(relationships["/fmt/645"]).to.equal(
          "Exchangeable Image File Format (Compressed) 2.21",
        );
        expect(relationships["/fmt/1507"]).to.equal(
          "Exchangeable Image File Format (Compressed) 2.3.x",
        );
      });
  });
});
