describe("PRONOM View Format Spec", () => {
    ...
    it("renders the signature section", () => {
        ...
        cy.get('#signatures-content-1 > dl > dt').each(dt => {
            ...
            expect(signatureFields["Position Type"]).to.equal("Absolute from BOF")
            expect(signatureFields["Offset"]).to.equal("0")
            expect(signatureFields["Maximum offset"]).to.equal("0")
            expect(signatureFields["Value"]).to.equal("4D565F41532D2D30")
        })
